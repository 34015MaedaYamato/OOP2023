    ...
    class Program {
        ...
        private static Novelist Exercise2_1(string v) {
            using (var reader = XmlReader.Create(v)) {
                ...
                var novelists = serializer.Deserialize(reader) as Novelist;
        ...
        private static void Exercise2_2(object novelist, string v) {
            using (var stream = new FileStream(v, FileMode.Create, FileAccess.Write)) {
                var serializer = new DataContractJsonSerializer(novelist.GetType(),new DataContractJsonSerializerSettings { DateTimeFormat = new DateTimeFormat("yyyy-MM-dd'T'HH:mm:ssZ") });
